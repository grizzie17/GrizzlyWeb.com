...
<xsl:template match="yweather:forecast">
	...
	<xsl:text> </xsl:text>
	<span class="weatherForecastHigh"><xsl:value-of select="@high"/><xsl:if test="@high != '-'"><xsl:text disable-output-escaping="yes">&amp;deg;</xsl:text></xsl:if></span> / <span class="weatherForecastLow"><xsl:value-of select="@low"/><xsl:if test="@low != '-'"><xsl:text disable-output-escaping="yes">&amp;deg;</xsl:text></xsl:if></span>
	</div>
</xsl:template>
