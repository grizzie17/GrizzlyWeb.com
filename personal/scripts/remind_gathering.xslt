...
<xsl:template match="event">
	<xsl:if test="local:isFirstRun()">
	<span style="font-weight:bold"><xsl:value-of select="local:buildDateString(.)"/></span><br/>
		<xsl:apply-templates select="subject"/>
		<xsl:apply-templates select="content"/>
		<xsl:apply-templates select="body"/>
		<xsl:apply-templates select="location"/>
		<xsl:apply-templates select="comments"/></xsl:if>
</xsl:template>
...
function buildDateString(oEvent)
	...
	if not oDate is nothing then
		...
		buildDateString = WEEKDAYNAME(wd,false,1) & ", " & getMonthName(mm,0) & " " & dd & ", " & yy
