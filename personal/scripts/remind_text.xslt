...
<xsl:template match="event"><xsl:apply-templates select="date"/>|<xsl:value-of select="local:buildDateString(.)"/>|<xsl:value-of select="local:buildWeekdayString(.)"/>|<xsl:apply-templates select="style"/><xsl:apply-templates select="subject"/><xsl:apply-templates select="content"/><xsl:apply-templates select="location"/><xsl:apply-templates select="comments"/><xsl:if test="style">}}</xsl:if>|<xsl:call-template name="br" /></xsl:template>

<xsl:template match="date">
	<xsl:if test="local:isNewMonth(.)">
<xsl:call-template name="br" />
!!! <xsl:value-of select="local:buildMonthString(.)"/>
<xsl:call-template name="br" />
<xsl:call-template name="br" />
	</xsl:if>
</xsl:template>
