...
FUNCTION htmlFormatMeta_f_big( aArgs(), sText )
	IF UBOUND(aArgs) < 1 THEN
		...
		htmlFormatMeta_f_big = "<span style=""font-size:larger"">" & sText & "</span>"
	ELSE
...
FUNCTION htmlFormatMeta_f_small( aArgs(), sText )
	IF UBOUND(aArgs) < 1 THEN
		...
		htmlFormatMeta_f_small = "<span style=""font-size:smaller"">" & sText & "</span>"
	ELSE
	...
	IF 0 < UBOUND(aArgs) THEN
		...
		FOR i = 1 TO UBOUND(aArgs)
			...
			CASE "large", "big"
				...
				sPrefix = sPrefix & "<span style=""font-size:larger"">"
				sPostfix = "</span>" & sPostfix
			CASE "small"
				'sPrefix = sPrefix & "<font size=""-1"">"
				'sPostfix = "</font>" & sPostfix
				sPrefix = sPrefix & "<span style=""font-size:smaller"">"
				sPostfix = "</span>" & sPostfix
			case "strike", "s"
				sPrefix = sPrefix & "<span style=""text-decoration:line-through"">"
				sPostfix = "</span>" & sPostfix
			CASE ELSE
				...
				IF 0 < LEN(sTemp) THEN
					...
					'sPostfix = "</font>" & sPostfix
					sPrefix = sPrefix & "<span style=""color:" & sTemp & """>"
					sPostfix = "</span>" & sPostfix
				ELSE
					sPrefix = ""
					EXIT FOR
